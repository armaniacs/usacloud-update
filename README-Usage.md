...
環境変数を自動検出し、設定ファイルへの移行を支援する新機能です。

- **環境変数自動検出**: `usacloud-update config` で既存の環境変数を自動識別
- **対話的ファイル生成**: 検出した環境変数から設定ファイルを簡単作成
- **完全テスト安定化**: `make test` で17テストパッケージが100%成功
- **厳密検証モード**: `--strict-validation` フラグによる高精度検証

### v1.9.0 基盤機能: サンドボックス統合

- **サンドボックス環境**: tk1v ゾーンでの料金なしテスト実行
- **インタラクティブTUI**: 直感的なコマンド選択・実行インターフェース
- **バッチ実行**: 全コマンドの自動実行モード
- **ドライランモード**: 実行せずに結果プレビュー
- **BDDテスト**: 行動駆動開発によるテスト自動化
